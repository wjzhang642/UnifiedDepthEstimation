...
if __name__ == '__main__':
    dataset_dir = './data/public_dataset/MVS-Synth'
    prefix = 'public_dataset/MVS-Synth/GTAV_720'

    data_path = osp.join(dataset_dir, 'GTAV_720')
    ...
    for idx, scene in enumerate(scenes):
        ...
        img_dir = osp.join(scene_path, 'images')
        gt_dir = osp.join(scene_path, 'depths')
        ...
        for image_name, gt_name, image_path, gt_path in zip(image_names, gt_names, image_paths, gt_paths):
            ...
            img_path = osp.join(prefix, scene, 'images', image_name)
            depth_gt_path = osp.join(prefix, scene, 'depths', gt_name)

            img_path = img_path.replace('\\', '/')
            depth_gt_path = depth_gt_path.replace('\\', '/')
            txt_content.append({'img_path': img_path, 'depth_gt_path': depth_gt_path, 'dataset_name': 'MVS-Synth'})

    save_file_name = 'train_public_MVS_Synth'
